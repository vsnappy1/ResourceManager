...
abstract class ResourceManagerMigrationTask : DefaultTask() {
    ...
    fun run() {
        ...
        val moduleManager = ModuleManager(project.projectDir)
    ...
    private fun getSourceFiles(): List<File> {
        val sourceDirectory = project.projectDir.resolve("src/main")
        val sourceFiles = project.fileTree(sourceDirectory).files.filter { it.isSourceFile() }
        return sourceFiles
    }
    ...
    private fun getNamespaceModuleMap(moduleManager: ModuleManager): Map<String, String> {
        ...
        moduleManager.getModuleDependencies().forEach { moduleName ->
            val module = project.projectDir.parentFile?.resolve(moduleName)
            if (module != null) {
